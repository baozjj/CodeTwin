...
import { DuplicateReport, CodeUnit, SimilarPair } from "./types";
import { DuplicateCodeLensProvider } from "./providers/codeLensProvider";
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  codeLensProvider = new DuplicateCodeLensProvider();
  outputChannel = vscode.window.createOutputChannel("CodeTwin");

  // 注册 CodeLens 提供者
  context.subscriptions.push(
    vscode.languages.registerCodeLensProvider(
      { scheme: "file", pattern: "**/*.{ts,tsx,js,jsx,vue}" },
      codeLensProvider
    )
  );
  ...
  // 注册命令: 提取代码
  context.subscriptions.push(
    vscode.commands.registerCommand("codetwin.extractCode", extractCodeUnits)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "codetwin.showDuplicates",
      showDuplicatesInteraction
    )
  );
 ...
 * 处理单个文件的增量查重
 */
async function processSingleFile(document: vscode.TextDocument) {
  ...
  const duplicates = similarityService.findDuplicatesForFile(
    filePath,
    threshold
  );
 ...
 * 交互式显示重复项
 */
async function showDuplicatesInteraction(
  sourceUnit: CodeUnit,
  similarUnits: SimilarPair[]
) {
  ...
  try {
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        outputChannel.appendLine(
          `✅ 提取完成! 共找到 ${units.length} 个代码单元`
        );
        vscode.window.showInformationMessage(
          `提取完成: ${units.length} 个单元`
        );
      }
    );
  } catch (e) {
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        for (const unit of units) {
          const fileDuplicates = similarityService.findDuplicatesForFile(
            unit.filePath,
            threshold
          );
        ...
        vscode.window.showInformationMessage(
          `CodeTwin: 发现 ${report.duplicatePairs} 处重复逻辑`
        );
      }
    );
  } catch (error) {
